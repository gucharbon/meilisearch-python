...
class TestUpdate:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_pending_update_default(self):
        """Tests waiting for an update with default parameters"""
        response = await self.index.add_documents([{'id': 1, 'title': 'Le Petit Prince'}])
    ...
    @pytest.mark.asyncio
    async def test_wait_for_pending_update_timeout(self):
        """Tests timeout risen by waiting for an update"""
        with pytest.raises(TimeoutError):
            await self.index.wait_for_pending_update(2, timeout_in_ms=0)

    @pytest.mark.asyncio
    async def test_wait_for_pending_update_interval_custom(self):
        """Tests call to wait for an update with custom interval"""
        response = await self.index.add_documents(self.dataset_json)
        assert 'updateId' in response
        start_time = datetime.now()
        wait_update = await self.index.wait_for_pending_update(
            response['updateId'],
            interval_in_ms=1000,
            timeout_in_ms=6000
        )
    ...
    @pytest.mark.asyncio
    async def test_wait_for_pending_update_interval_zero(self):
        """Tests call to wait for an update with custom interval"""
        response = await self.index.add_documents(self.dataset_json)
        assert 'updateId' in response
        wait_update = await self.index.wait_for_pending_update(
